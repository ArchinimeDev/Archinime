 ...
 * - Video SIN BORDES NEGROS en móvil (object-fit: cover)
...
const SPECIAL_VIDEOS = [
  ...
  // Puedes añadir más: 'assets/videos/otro.mp4'
];
...
function activarModoEspecial() {
  ...
  console.log('🌟 MODO ESPECIAL ACTIVADO (FULLSCREEN - SIN BORDES)');
  ...
  // En móvil usamos 100dvh para que se ajuste al viewport dinámico (sin bordes de navegador)
  ...
  // 🔥 USAMOS 'cover' SIEMPRE: así rellena toda la pantalla sin bordes negros
...
window.addEventListener('resize', () => {
  if (window.isSpecialMode) {
    ...
    if (video && banner) {
      // Siempre 'cover' para eliminar bordes negros
      video.style.objectFit = 'cover';
      // Actualizar altura dinámica (dvh es mejor para móviles)
      banner.style.height = isMobileDevice() ? '100dvh' : '100vh';
    }
  }
});
